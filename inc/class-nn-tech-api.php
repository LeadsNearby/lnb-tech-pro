...
if (!class_exists('NNTechAPI')):
  ...
  class NNTechAPI {
    ...
    public static function get_nn_data() {
      ...
      $nn_api_key = get_option('nearbynow_options')['text_string'];
      ...
      if (!$data) {
        ...
        if ($tech_query->have_posts()):
          while ($tech_query->have_posts()): $tech_query->the_post();
            ...
            $url = 'https://api.sidebox.com/plugin/nearbyserviceareareviewcombo/?storefronttoken=' . $nn_api_key . '&reviewcount=0&checkincount=0&techemail=' . $tech_email;
...
add_action('save_post_profiles', array('NNTechAPI', 'clear_data'));
// add_action('wp_update_nav_menu', array('NN_Tech_API', 'clear_data'));
add_action('after_rocket_clean_cache_dir', array('NNTechAPI', 'clear_data'));